...
object FoldMemoryUsage extends MemoryUsageApp {
  ...
  private def touchCode() {
    ...
    if (OptionalLong(10).fold(15L)(x => x + 1) != 11) sys.error("Fold not producing expected values")
    if (customOptionValues(0) != 0) sys.error("Custom Option Values Not Initialized Properly")
    initMemory()
  }
  ...
  def runCustomOptionsTest(pass: Int) {
    ...
    while (i < limit) {
      ...
      sum += OptionalLong(optional).fold(-150L)(x => x + 5)
